...
const UserDashboard = () => {
    ...
    const fetchSubscriptionStatus = async () => {
      try {
        const response = await fetch(
          `/api/getSubscriptionStatus?email=${userData.email}`
        );
  ...
  useEffect(() => {
    const fetchPaymentHistory = async () => {
      try {
        const response = await fetch(
          `/api/getPaymentHistory?email=${userData.email}`
        );
        const data = await response.json();
  ...
  const handleCancelSubscription = async (paymentId) => {
    try {
      ...
      if (data.success) {
        // Handle successful cancellation, like updating UI or showing a success message
        alert("Subscription cancelled successfully.");
        // Optionally, you can refetch payment history and update the UI
      } else {
    ...
    <table className="min-w-full text-sm text-gray-700">
      <thead className="bg-gray-300 text-black uppercase">
        <tr>
          ...
          <th className="py-3 px- text-center">Customer</th>
      ...
      <tbody>
        {paymentHistory.map((payment) => (
          <tr key={payment.paymentId} className="hover:bg-gray-200">
            ...
            <td
              className={`py-3 px-6 ${
                payment.customerStatus === "active"
                  ? "text-green-600"
                  : "text-red-600"
              }`}
            >
              {payment.customerStatus.charAt(0).toUpperCase() +
                payment.customerStatus.slice(1)}
            </td>
            <td className="py-3 px-6 text-center">{payment.customerName}</td>
            ...
            <td className="py-3 px-6 text-center">
              ...
              {payment.product === "tracker permiuem" ? (
                <button
                  onClick={() => handleSubscribe("basic")}
